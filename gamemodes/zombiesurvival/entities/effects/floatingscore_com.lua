-- ============================================================================
-- floatingscore_com.lua - 任务积分漂浮文字特效（客户端）
-- 负责：玩家获得任务积分时在目标位置显示黄色"XX point(s)"积分数字，
--       数字缓慢上飘、随寿命渐隐并缩小
-- ============================================================================
...
function EFFECT:Render()
	...
	cam_Start3D2D(self.Pos + math_sin(CurTime() + self.Seed) * 30 * delta * right, ang, (delta * 0.12 + 0.045) / 2)
		-- 整数积分显示"XX point(s)"（注意单复数）；小数拆为两部分显示
