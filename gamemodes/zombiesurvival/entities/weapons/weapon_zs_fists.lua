...
SWEP.PrintName = ""..translate.Get("weapon_zs_fists")
...
SWEP.IdleAngle = Angle(0, 0, 0)
SWEP.ActiveAngleLeft = Angle(-25, 25, -25)
SWEP.ActiveAngleRight = Angle(25, -10, 25)
...
SWEP.BlockSound = "physics/flesh/flesh_strider_impact_bullet"..math.random(3)..".wav"
SWEP.BlockSoundPitch = 90
...
function SWEP:Think()
   ...
   -- self:ApproachBlockAngles()
	...
	-- Set the default angle to idle
    self.AppToRight = self.IdleAngle
    self.AppToLeft = self.IdleAngle
     
     if self:IsBlocking() and self.Owner:KeyDown(IN_RELOAD) then
            self.AppToRight = self.ActiveAngleRight
            self.AppToLeft = self.ActiveAngleLeft
    end

    -- Smoothly approach the target angle for the right clavicle
     if CLIENT then
        ApproachAngle(self.ViewModelBoneMods["ValveBiped.Bip01_R_Clavicle"].angle, self.AppToRight, FrameTime() * 66)
        ApproachAngle(self.ViewModelBoneMods["ValveBiped.Bip01_L_Clavicle"].angle, self.AppToLeft, FrameTime() * 66)
    end
...
function SWEP:ApproachBlockAngles()
    ...
    if not self.CurrentBlockAngles then
        self.CurrentBlockAngles = {
            ["ValveBiped.Bip01_L_Clavicle"] = Angle(0, 0, 0),
            ["ValveBiped.Bip01_R_Clavicle"] = Angle(0, 0, 0),
        }
    end

    -- Loop through each bone and approach its target angle
    for bone, data in pairs(blockMods) do
        local targetAngle = data.angle
        local currentAngle = self.CurrentBlockAngles[bone]

        -- Approach each component of the angle (pitch, yaw, roll) separately
        currentAngle.p = math.Approach(currentAngle.p, targetAngle.p, FrameTime() * 10) 
        currentAngle.y = math.Approach(currentAngle.y, targetAngle.y, FrameTime() * 10)
        currentAngle.r = math.Approach(currentAngle.r, targetAngle.r, FrameTime() * 10)

        -- Save the updated angles back to the table
