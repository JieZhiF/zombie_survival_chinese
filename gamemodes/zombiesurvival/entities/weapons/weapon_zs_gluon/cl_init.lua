-- ============================================================================
-- weapon_zs_gluon/cl_init.lua - 胶子炮（Gluon，客户端）
-- 负责：定义武器槽位、第一/第三人称附加模型拼装（EGON 底座 + 各类零件），
--       以及屏幕与枪身上的过热（热量）HUD 绘制
-- ============================================================================
INC_CLIENT()
...
-- 枪身 3D2D HUD 的挂点骨骼与偏移/缩放
SWEP.HUD3DBone = "Base"
SWEP.HUD3DPos = Vector(3, -0.5, -13)
SWEP.HUD3DAng = Angle(180, 0, 0)
SWEP.HUD3DScale = 0.03

-- 第一人称附加模型：以洗衣机（egon_base）为主体，用各类零件拼出胶子炮外形
SWEP.VElements = {
...
-- ==== DrawHeatBar - 绘制过热条：按短/长时热量分两段（橙黄=短时、红=长时），过热排气时闪烁提示 ====
...
-- ==== Draw2DHUD - 绘制 2D 屏幕 HUD：左上角过热条 + 右下角剩余弹药/热量框 ====
...
-- ==== Draw3DHUD - 在枪身上绘制 3D2D HUD（同样的弹药数字与过热条） ====
