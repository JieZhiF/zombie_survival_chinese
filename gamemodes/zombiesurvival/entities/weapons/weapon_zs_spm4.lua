...
function SWEP:ShootBullets(dmg, numbul, cone)
    ...
    -- 视角后坐力
    if self.Recoil and self.Recoil > 0 then
    ...
    local GUARANTEE_RANGE = 600
    local WALL_PENETRATION = 80
    local startpos = owner:GetShootPos()

    -- 可配置排除阵营表
    local EXCLUDED_TEAMS = {
        [TEAM_SURVIVORS] = true,
        -- [TEAM_POLICE] = true,  -- 举例：要排除警察阵营就加上
    }

    -- ---------- 锁定玩家逻辑（只在服务器判断和造成伤害） ----------
    if SERVER then
        local entsInRange = ents.FindInSphere(startpos, GUARANTEE_RANGE)
        local bestTarget, bestDistSq = nil, math.huge

        for _, ent in ipairs(entsInRange) do
            if IsValid(ent) and ent:IsPlayer() and ent:Alive() then
                -- ⚠️ 排除自己 + 排除表 + 同阵营
                if ent ~= owner and not EXCLUDED_TEAMS[ent:Team()] and owner:Team() ~= ent:Team() then
                    local d2 = startpos:DistToSqr(ent:WorldSpaceCenter())
        ...
        if IsValid(bestTarget) then
            local targetPos = bestTarget:WorldSpaceCenter()
            ...
            if not tr.Hit or tr.Entity == bestTarget then
                -- 直线可见，直接命中
                ...
                bestTarget:TakeDamageInfo(dmginfo)
                ...
                -- 被墙挡住，检查厚度
                local wallThickness = tr.HitPos:Distance(targetPos)
                if wallThickness <= WALL_PENETRATION then
                    local dmginfo = DamageInfo()
                    dmginfo:SetAttacker(owner)
                    dmginfo:SetInflictor(self)
                    dmginfo:SetDamage(dmg)
