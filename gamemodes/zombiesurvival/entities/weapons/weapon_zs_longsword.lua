...
SWEP.PrintName = ""..translate.Get("weapon_zs_longsword")
SWEP.Description = ""..translate.Get("weapon_zs_longsword_description")
...
SWEP.BlockPos = Vector(-13.867, -15.54, 7.62)
SWEP.BlockAng = Angle(5.133, 3.448, -57.561)

SWEP.DefendingDamageBlocked = 2.7
SWEP.DefendingDamageBlockedDefault = 2.7
...
SWEP.ThirdAbilityConfig = {
    Icon = Material("materials/ultimate.png"),
    IconSize = 60,
    IconColor = Color(255, 150, 150, 255), -- 补上之前缺失的图标颜色
    BGColor = Color(90, 0, 0, 150),
    CooldownColor = Color(255, 50, 50, 255),
    GlowColor = Color(255, 170, 170), -- 只提供基础颜色
    TextLines = {
        { text = translate.Get("meleehud_passive"), y_off = 10, color = Color(255, 170, 170) }
    },
    GetRatio = function(wep, owner, w, h)
        local r = (owner:Health() <= owner:GetMaxHealth() * 0.25) and 0 or 1
        local healthRemaining = math.max(owner:Health() - (owner:GetMaxHealth() * 0.25), 0)
        local maxHealthAbove25 = owner:GetMaxHealth() * 0.75
        local cH = h * (1 - (healthRemaining / maxHealthAbove25))
        return r, cH
    end
}
...
function SWEP:DrawCustomHUD(owner, scrW, scrH)
	if owner:Health() <= owner:GetMaxHealth() * 0.25 then
        local text_color = Color(170,20,20)
        local text_pos_y = scrH - 180
		draw.SimpleText(translate.Get("meleehud_2xdamage"), "ZSM_Coolvetica", scrW / 2, text_pos_y, text_color, TEXT_ALIGN_CENTER)
		draw.SimpleText(translate.Get("meleehud_2xdamage"), "ZSM_CoolveticaBlur", scrW / 2, text_pos_y, text_color, TEXT_ALIGN_CENTER)
	end
end