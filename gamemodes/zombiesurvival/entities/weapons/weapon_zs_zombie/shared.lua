...
SWEP.PrintName = ""..translate.Get("weapon_zs_zombie")
...
function SWEP:GetDamage(numplayers, basedamage)
	if self.Owner.m_Zombie_Damage1 then
		updmg = self.MeleeDamage * 1.07
		takedmg = updmg
	end

	basedamage = takedmg or self.MeleeDamage 
...
function SWEP:PrimaryAttack()
	...
	local owner = self:GetOwner()
	-- 完全冻结状态下无法攻击
	if owner:IsFrozenFull() then return end
