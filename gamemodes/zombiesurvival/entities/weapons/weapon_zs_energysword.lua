...
SWEP.PrintName = translate.Get("weapon_zs_energysword")
SWEP.Description = translate.Get("weapon_zs_energysword_description")
...
function SWEP:CanPrimaryAttack()
	...
	if self:GetOwner():GetAmmoCount("pulse") < 1 then return false end
		...
		if trace.Hit then
			...
			if not (trace.HitWorld and (trace.HitNormal.z > 0.8)) then
				...
				if ent and ent:IsValid() then
					...
					if SERVER then
						self:GetOwner():GiveAmmo(10, "pulse")
