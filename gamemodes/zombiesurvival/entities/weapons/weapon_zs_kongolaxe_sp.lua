...
local function CountRealPlayersInTeam(teamid)
	local count = 0

	for _, ply in ipairs(player.GetAll()) do
		if ply:Team() == teamid and ply:IsPlayer() and not ply:IsBot() then
			count = count + 1
		end
	end
...
function SWEP:GetMeleeDamage(ent, tr)
	local dmg = self.MeleeDamage

	-- 仅服务器端生效
	if SERVER then
		local survivors = CountRealPlayersInTeam(TEAM_SURVIVORS)
		local zombies   = CountRealPlayersInTeam(TEAM_ZOMBIE)

		if survivors == 1 and zombies == 0 then
			dmg = dmg * 1.8
		end
	end
