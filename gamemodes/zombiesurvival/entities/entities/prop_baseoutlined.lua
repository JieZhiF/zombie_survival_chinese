...
function ENT:DrawTranslucent()
	...
	if self.QualityTier then
		...
		if customcols then
            ...
            -- ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼【核心修改区域】▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            -- 1. 先安全地获取质量颜色表
			local quality_color_table = GAMEMODE.WeaponQualityColors[self.QualityTier]
            
            -- 2. 检查这个表是否存在
			if quality_color_table then
                -- 3. 如果存在，才继续执行原始逻辑
				local qcolt = quality_color_table[self.Branch and 2 or 1]
