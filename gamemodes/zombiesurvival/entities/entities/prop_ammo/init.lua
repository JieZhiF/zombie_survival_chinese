...
function ENT:SetAmmoType(ammotype)
    local lowerType = string.lower(ammotype) -- 转换为小写
    self:SetModel(GAMEMODE.AmmoModels[lowerType] or "models/Items/BoxMRounds.mdl")
    self.m_AmmoType = lowerType
    self:SetNWString("AmmoType", lowerType) -- 同步小写值
end
...
function ENT:SetAmmo(ammo)
	...
	self:SetNWString("AmmoAmount", ammo) -- 同步小写值
end
