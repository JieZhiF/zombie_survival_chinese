...
function ENT:Initialize()
	...
	-- 2. 定义各个轴的缩放比例
	-- Vector(x, y, z) 分别对应 X轴(宽度), Y轴(深度), Z轴(高度) 的缩放倍数
	-- 这里我们将 X 和 Y 设为 0.55，将 Z (竖直方向) 设为 1.2
	...
	if cmodel:IsValid() then
		...
		--[[
	local thumperModel = ClientsideModel("models/props_combine/combinethumper002.mdl")

	if thumperModel:IsValid() then

		thumperModel:SetPos(self:LocalToWorld(Vector(0, 0, -11)))
		thumperModel:SetAngles(self:LocalToWorldAngles(Angle(0, 0, 0)))
		thumperModel:SetSolid(SOLID_NONE)
		thumperModel:SetMoveType(MOVETYPE_NONE)
		thumperModel:SetColor(Color(255, 150, 150))
		thumperModel:SetParent(self)
		thumperModel:SetOwner(self)

		local matrix = Matrix()
		matrix:Scale(Vector(0.6, 0.6, 0.02))
		thumperModel:EnableMatrix( "RenderMultiply", matrix )

		thumperModel:Spawn()
		self.ThumperModel = thumperModel -- 将其存储以便后续引用
	end
	]]
	local healmodel = ClientsideModel("models/healthvial.mdl")
	if healmodel:IsValid() then
		healmodel:SetPos(self:LocalToWorld(Vector(0.5, 0, 0.4)))
		healmodel:SetAngles(self:LocalToWorldAngles(Angle(0, 0, 0)))
		healmodel:SetSolid(SOLID_NONE)
		healmodel:SetMoveType(MOVETYPE_NONE)
		--healmodel:SetColor(Color(255, 150, 150))
		healmodel:SetParent(self)
		healmodel:SetOwner(self)
		
		local matrix = Matrix()
		matrix:Scale(Vector(2.1,2.1,3.5))
		healmodel:EnableMatrix( "RenderMultiply", matrix )
		
		healmodel:Spawn()

		self.healmodel = healmodel
	end
end
...
function ENT:Think()
	...
	if CurTime() >= self.NextEmit and self:GetAmmo() > 0 then
	...
	if MySelf:IsValid() then
		if self.Pulsed then
			if CurTime() < self:GetNextMedicPulse() then
				self.Pulsed = false
			end
		elseif CurTime() >= self:GetNextMedicPulse() then
			self.Pulsed = true
...
function ENT:OnRemove()
	...
	if self.healmodel and self.healmodel:IsValid() then
		self.healmodel:Remove()
	end
end
