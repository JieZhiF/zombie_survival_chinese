-- ============================================================================
-- sh_serialization.lua - 通用序列化工具（SRL 格式）
-- 负责：将 Lua table 序列化为可存储/传输的字符串（Serialize），
--       以及将 "SRL={...}" 格式字符串反序列化回 table（Deserialize）；
--       支持 string/number/boolean/Vector/Angle/嵌套 table，
--       带循环引用保护与沙盒执行环境，防止反序列化时执行恶意代码。
--       地图配置、玩家档案等持久化数据均使用本格式（见 profiler_premade/ 等）。
-- ============================================================================
