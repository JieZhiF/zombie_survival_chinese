-- ============================================================================
-- sh_zombieescape.lua - 僵尸逃跑（Zombie Escape）模式共享逻辑
-- 负责：仅在 ze_ 开头的地图中生效（否则整文件跳过）；配置 ZE 专属参数
--       （准备波时长、冻结时间、时间上限等）、覆盖移动与伤害缩放规则、
--       注册 CSS 武器与虚拟实体（防止控制台报错刷屏），
--       并限制 ZE 模式下的武器拾取规则（按阵营/武器种类过滤）。
-- ============================================================================
